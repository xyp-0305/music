(function (global, factory) {
  ...
  (global = global || self, factory(global.XTemplateRuntime = {}));
  ...
  var win = typeof global !== 'undefined' ? global : self;
  ...
  var util = {
    ...
    each: function each(object, fn, context) {
      if (context === void 0) {
        context = null;
      }
    ...
    splitPath: function splitPath(str) {
  ...
  var commands = {
    ...
    forin: function forin(scope, option, buffer_) {
      ...
      var opScope;
      var affix;
      var name; // if undefined, will emit warning by compiler
    ...
    macro: function macro(scope, option, buffer_) {
      ...
      var macro = macros[macroName]; // definition
      ...
      if (option.fn) {
        ...
        var newScope = new Scope(paramValues); // https://github.com/xtemplate/xtemplate/issues/29

        newScope.root = scope.root; // no caller Scope
   ...
   */
  var commands$1 = {};
  var splitPath = util.splitPath; // for performance: reduce hidden class

  function TplWrap(name, runtime, root, scope, buffer, originalName, fn, parent) {
    this.name = name;
    this.originalName = originalName || name;
    this.runtime = runtime;
    this.root = root; // line counter
  ...
  function findCommand(runtimeCommands, instanceCommands, parts) {
    var name = parts[0];
    var cmd = runtimeCommands && runtimeCommands[name] || instanceCommands && instanceCommands[name] || commands$1[name];
  ...
  function getSubNameFromParentName(parentName, subName) {
    ...
    var parts = splitPath(parentName);
    var subParts = splitPath(subName);
    parts.pop();
  ...
  function loadInternal(root, name, runtime, scope, buffer, originalName, escape, parentTpl) {
    ...
    root.config.loader.load(tpl, function (error, tplFn_) {
      ...
      if (typeof tplFn === 'function') {
        tpl.fn = tplFn; // reduce count of object field for performance
  ...
  function renderTpl(tpl) {
    var buffer = tpl.fn(); // tpl.fn exception
    ...
    if (buffer) {
      ...
      var extendTplBuffer = runtime.extendTplBuffer; // if has extend statement, only parse
  ...
  exports.default = XTemplateRuntime;
